...
- IPVS 规则查看：`ipvsadm -Ln`
- iptables 规则查看：`iptables -L`, `iptables -t nat -L`
- 不能使用 ss 或 netstat 查看连接，因为 IPVS 工作在内核层，不暴露为本地 socket。
- 查看 IPVS 连接状态	ipvsadm -Lnc
- 查看连接统计/速率	ipvsadm -Ln --stats/--rate
- 查看内核原始数据	/proc/net/ip_vs_conn
- 查看 NAT/防火墙关联	conntrack -L